<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up()
    {
        Schema::table('lesson_lectures', function (Blueprint $table) {
            $table->string('content_type')->default('text')->after('content'); // 'text' або 'file'
            $table->string('file_path')->nullable()->after('content_type');
            $table->string('file_type')->nullable()->after('file_path');
            $table->string('file_name')->nullable()->after('file_type');
        });

        Schema::table('lesson_extra_materials', function (Blueprint $table) {
            $table->string('file_type')->nullable()->after('file_path');
            $table->string('file_name')->nullable()->after('file_type');
        });
    }

    public function down()
    {
        Schema::table('lesson_lectures', function (Blueprint $table) {
            $table->dropColumn(['content_type', 'file_path', 'file_type', 'file_name']);
        });

        Schema::table('lesson_extra_materials', function (Blueprint $table) {
            $table->dropColumn(['file_type', 'file_name']);
        });
    }
};
